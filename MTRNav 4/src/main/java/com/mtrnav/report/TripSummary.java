...
public final class TripSummary {
	...
	public static final class LegSummary {
		...
		private LegSummary(boolean isWalk, String lineName, String boardName, String alightName, String terminusName,
		...
		static LegSummary from(RouteLeg leg) {
			...
			if (leg.isWalk()) {
				return new LegSummary(true, "Walk", leg.boardPlatform.getName(), leg.alightPlatform.getName(), "", passed, leg.durationMillis);
			}
			return new LegSummary(false, leg.lineName(), leg.boardPlatform.getName(), leg.alightPlatform.getName(),
